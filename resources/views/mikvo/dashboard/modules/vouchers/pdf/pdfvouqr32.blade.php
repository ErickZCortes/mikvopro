    ...
    <table class="table"> @foreach($detailv->chunk(4) as $detail)  
        <tr> @foreach($detail as $detaili)
            ...
            <table class="inside" style="background-color: {{$template->bgcolor_template}};">
                ...
                <tr>
                    <td style="font-size: 10px; font-family:{{$template->font_template}}; background:{{$template->color_voucher}}; color: white; text-align: center;">{{$voucher->nprofile_voucher}}</td>
                </tr>
                <tr>
